    ...
    public sealed class CoopGame : BaseLocalGame<EftGamePlayerOwner>, IBotGame, IFikaGame
    {
        ...
        private IEnumerator WaitForOtherPlayers()
        {
            if (CoopHandler.TryGetCoopHandler(out CoopHandler coopHandler))
            {
                ...
                if (isServer)
                {
                    ...
                    FikaPlugin.Instance.FikaLogger.LogDebug("[CoopGame] Waiting for host status update as server");
                    while (!updateStatus.IsCompleted)
                    {
                        yield return null;
                    }

                    FikaPlugin.Instance.FikaLogger.LogDebug("[CoopGame] Potentially infinite loop, waiting for players as server");
                    do
                    {
                        yield return null;
        ...
        private async Task WaitForPlayers()
        {
            ...
            if (isServer)
            {
                ...
                if (!server.Groups.TryGetValue(FikaGroupUtils.GroupId, out var groupInfo))
                {
        ...
        public override IEnumerator vmethod_4(BotControllerSettings controllerSettings, ISpawnSystem spawnSystem, Callback runCallback)
        {
            ...
            {
                FikaServer server = Singleton<FikaServer>.Instance;
                var readyPlayers = server.Groups[FikaGroupUtils.GroupId];
                while (readyPlayers.ReadyClients < expectedPlayers)
                {
                    readyPlayers = server.Groups[FikaGroupUtils.GroupId];
                    yield return new WaitForEndOfFrame();
                }
            }
            else
